...
function App() {
  ...
  const fetchTasks = async (t = token, fetchAll = false) => {
    if (!t) return;
    try {
      const url = fetchAll || viewMode === 'responsibility'
        ? `${API_URL}/tasks?mode=all`
        : `${API_URL}/tasks`;
  ...
  const openNewTaskModal = (dateStr, timeStr) => {
    setEditingTask({
      _id: null,
      title: '',
      tag: defaultSettings.defaultTag,
      isEvent: defaultSettings.defaultType === 'event',
      isAllDay: !timeStr,
      priority: 4,
      dueDate: dateStr,
      startTime: timeStr || '09:00',
      endTime: timeStr ? calculateEndTime(timeStr) : '10:00',
      description: '',
      subtasks: [],
      attachments: [],
      list_id: currentList ? currentList._id : null,
    });
  };
  ...
  const saveTaskChanges = async () => {
    ...
    try {
      const payload = {
        ...editingTask,
        isDone: editingTask.isEvent ? false : editingTask.isDone,
      };
      if (editingTask._id) {
        await axios.put(
          `${API_URL}/tasks/${editingTask._id}/update`,
          payload,
          getAuthHeader(),
        );
      } else {
  ...
  const quickAddTask = async (e) => {
    e.preventDefault();
    if (!newTask.trim()) {
      openNewTaskModal(new Date().toISOString().split('T')[0], '09:00');
      return;
    }
    try {
      await axios.post(
  ...
  const updateList = async (id) => {
    ...
    try {
      const res = await axios.put(
        `${API_URL}/lists/${id}`,
        { name: editingListTitle, owner_id: '' },
        getAuthHeader(),
      );
  ...
  const getVisibleTasks = () => {
    let rel = viewMode === 'trash'
      ? tasks.filter((t) => t.isDeleted)
      : tasks.filter((t) => !t.isDeleted);
    if (currentList) rel = rel.filter((t) => t.list_id === currentList._id);
    if (searchQuery) rel = rel.filter((t) => t.title.toLowerCase().includes(searchQuery.toLowerCase()));
    return rel;
  };
  const visibleTasks = getVisibleTasks();
  const saveSettings = (n) => {
    setDefaultSettings(n);
    localStorage.setItem('userSettings', JSON.stringify(n));
  };
  const changeDateBase = (amt) => {
    const d = new Date(currentDateBase);
    if (viewMode === 'timeline') d.setDate(d.getDate() + amt);
    else if (viewMode === 'month') d.setMonth(d.getMonth() + amt);
    else if (viewMode === 'year') d.setFullYear(d.getFullYear() + amt);
    setCurrentDateBase(d);
  };
  ...
  const addSubtask = () => {
    setEditingTask({
      ...editingTask,
      subtasks: [...(editingTask.subtasks || []), { title: '', isDone: false }],
    });
  };
  const toggleSubtask = (i) => {
    const u = [...editingTask.subtasks];
    u[i].isDone = !u[i].isDone;
    setEditingTask({ ...editingTask, subtasks: u });
  };
  const updateSubtaskTitle = (i, v) => {
    const u = [...editingTask.subtasks];
    u[i].title = v;
    setEditingTask({ ...editingTask, subtasks: u });
  };
  ...
  const handleTimelineDrop = async (
    ...
    const u = { ...draggedTask, dueDate: dateStr };
    if (isAllDayDrop) {
      u.isAllDay = true;
    } else {
      u.isAllDay = false;
    ...
    try {
      await axios.put(
        `${API_URL}/tasks/${draggedTask._id}/update`,
        u,
        getAuthHeader(),
      );
    } catch {
  ...
  const handleMatrixDrop = async (e, pri) => {
    e.preventDefault();
    if (!draggedTask) return;
    const u = { ...draggedTask, priority: pri };
    setTasks(tasks.map((t) => (t._id === draggedTask._id ? u : t)));
    try {
      await axios.put(
        `${API_URL}/tasks/${draggedTask._id}/update`,
  ...
  const handleResponsibilityDrop = async (e, assigneeName) => {
    e.preventDefault();
    if (!draggedTask) return;
    const u = { ...draggedTask, assignee: assigneeName };
    setTasks(tasks.map((t) => (t._id === draggedTask._id ? u : t)));
    try {
      await axios.put(
        `${API_URL}/tasks/${draggedTask._id}/update`,
