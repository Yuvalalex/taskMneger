      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="type-switcher">
            <button
              className={!editingTask.isEvent ? 'active' : ''}
              onClick={() => setEditingTask({ ...editingTask, isEvent: false })}
            >
              Task
            </button>
            <button
              className={editingTask.isEvent ? 'active' : ''}
              onClick={() => setEditingTask({ ...editingTask, isEvent: true })}
            >
              Event
            </button>
          </div>
          ...
          <div className="priority-selector">
            ...
            {[1, 2, 3, 4].map((p) => (
              <button
                key={p}
                className={`p-btn p${p} ${editingTask.priority === p ? 'active' : ''}`}
